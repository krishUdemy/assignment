...
import { cancelInterceptorFactory, cancelPreviousOnNewRequestInterceptorFactory, addAcceptLanguageHeaderInterceptorFactory } from './interceptors.js';
...
export class AjaxClass {
  ...
  request(url, config) {
    return this.proxy.request.apply(this, [url, { ...this.__config,
      ...config
    }]);
  }
  /**
  ...
  get(url, config) {
    return this.proxy.get.apply(this, [url, { ...this.__config,
      ...config
    }]);
  }
  /**
  ...
  delete(url, config) {
    return this.proxy.delete.apply(this, [url, { ...this.__config,
      ...config
    }]);
  }
  /**
  ...
  head(url, config) {
    return this.proxy.head.apply(this, [url, { ...this.__config,
      ...config
    }]);
  }
  /**
  ...
  post(url, data, config) {
    return this.proxy.post.apply(this, [url, data, { ...this.__config,
      ...config
    }]);
  }
  /**
  ...
  put(url, data, config) {
    return this.proxy.put.apply(this, [url, data, { ...this.__config,
      ...config
    }]);
  }
  /**
  ...
  patch(url, data, config) {
    return this.proxy.patch.apply(this, [url, data, { ...this.__config,
      ...config
    }]);
  }

  __setupInterceptors() {
    this.proxy.interceptors.request.use(config => {
    ...
    this.proxy.interceptors.response.use(response => this.responseInterceptors.reduce((r, i) => i(r), response), error => {
  ...
  __setupTransformers(config) {
    ...
    return { ...config,
      transformRequest: (data, headers) => {
        try {
          const ourData = this.requestDataTransformers.reduce((d, t) => t(d, headers), data); // axios does a lot of smart things with the request that people rely on
