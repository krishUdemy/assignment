import { dedupeMixin, html, SlotMixin } from "../../../core/index.js";
import { DisabledMixin } from "../../../core/src/DisabledMixin.js";
...
export const FormGroupMixin = dedupeMixin(superclass => // eslint-disable-next-line no-shadow
class FormGroupMixin extends FormRegistrarMixin(FormControlMixin(ValidateMixin(DisabledMixin(SlotMixin(superclass))))) {
  ...
  _anyFormElementHasFeedbackFor(state) {
    return Object.keys(this.formElements).some(name => {
      ...
      return Boolean(this.formElements[name].hasFeedbackFor && this.formElements[name].hasFeedbackFor.includes(state));
    });
  }
  ...
  __linkChildrenMessagesToParent(child) {
    ...
    while (parent) {
      this.constructor._addDescriptionElementIdsToField(child, parent._getAriaDescriptionElements()); // Also check if the newly added child needs to refer grandparents
