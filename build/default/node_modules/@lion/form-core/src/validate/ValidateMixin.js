...
import { dedupeMixin, ScopedElementsMixin, SlotMixin } from "../../../core/index.js"; // TODO: make form-core independent from localize
...
export const ValidateMixin = dedupeMixin(superclass => // eslint-disable-next-line no-unused-vars, no-shadow
class ValidateMixin extends SyncUpdatableMixin(SlotMixin(ScopedElementsMixin(superclass))) {
  ...
  static get properties() {
    return {
      ...
      validationStates: {
        type: Object // hasChanged: this._hasObjectChanged,
  ...
  get slots() {
    return { ...super.slots,
      feedback: () => document.createElement(this.constructor.getScopedTagName('lion-validation-feedback'))
    };
  }
  /**
   * @overridable
  ...
  async __executeValidators() {
    ...
    const
    /** @type {Validator[]} */
    syncValidators = filteredValidators.filter(v => !v.constructor.async);
    const
    /** @type {Validator[]} */
    asyncValidators = filteredValidators.filter(v => v.constructor.async);
    /**
  ...
  __executeResultValidators(regularValidationResult) {
    ...
    const resultValidators = this._allValidators.filter(v => !v.constructor.async && v instanceof ResultValidator);
  ...
  __finishValidation({
    ...
    const
    /** @type {Validator[]} */
    syncAndAsyncOutcome = [...this.__syncValidationResult, ...this.__asyncValidationResult]; // if we have any ResultValidators left, now is the time to run them...
    ...
    const validationStates = this.constructor.validationTypes.reduce((acc, type) => ({ ...acc,
      [type]: {}
    }), {});
  ...
  __setupValidators() {
    ...
    this._allValidators.forEach(v => {
      if (!(v instanceof Validator)) {
        ...
        const errorMessage = `Validators array only accepts class instances of Validator. Type "${errorType}" found.`; // eslint-disable-next-line no-console
      ...
      if (this.constructor.validationTypes.indexOf(v.type) === -1) {
        // throws in constructor are not visible to end user so we do both
        const errorMessage = `This component does not support the validator type "${v.type}" used in "${v.constructor.validatorName}". You may change your validators type or add it to the components "static get validationTypes() {}".`; // eslint-disable-next-line no-console
  ...
  updated(changedProperties) {
    ...
    if (changedProperties.has('shouldShowFeedbackFor') || changedProperties.has('hasFeedbackFor')) {
      this.showsFeedbackFor = this.constructor.validationTypes.map(type => this._hasFeedbackVisibleFor(type) ? type : undefined).filter(_ => !!_);
  ...
  _updateShouldShowFeedbackFor() {
    this.shouldShowFeedbackFor = this.constructor.validationTypes.map(type => this._showFeedbackConditionFor(type) ? type : undefined).filter(_ => !!_);
  }
  /**
   * @overridable
  ...
  _prioritizeAndFilterFeedback({
    ...
    const types = this.constructor.validationTypes; // Sort all validators based on the type provided.
